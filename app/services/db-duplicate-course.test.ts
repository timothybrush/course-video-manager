...
/**
 * Schema-drift guard. For every table the duplicate walks, we declare which
 * columns are *copied* (content that must carry over) and which are *not copied*
 * (identity, parent FKs, lineage, timestamps, reset flags). The union must equal
 * the table's actual columns — so adding a new column to `schema.ts` without
 * categorizing it here fails the build, forcing a conscious decision instead of a
 * silent slippage. Every "copied" column is also value-checked end-to-end against
 * a fixture that sets each column to a distinctive, non-default value.
 */
describe("duplicateCourse — schema-drift guard", () => {
  // Each copied column below is set to a distinctive, non-default value so the
  // value-check would fail if the duplicate stopped carrying it over.
  const COPY_SPEC = {
    section: {
    ...
    video: {
      table: schema.videos,
      copied: ["title", "originalFootagePath", "body", "description"],
      notCopied: [
        "id",
        "lessonId",
        "pitchId",
  ...
  // Fetch a single source row by an equality filter, for value comparison.
  ...
  it("carries over every copied column end-to-end", async () => {
    // Build a source course where every copied column has a distinctive value.
